...
func TestDenyIsOneKeyAndAllowStopsToConfirm(t *testing.T) {
	...
	// consent flow gave deny `--server` and a passphrase; now agent.pending
	// and agent.show declare it per action, and this test keeps the declaration
...
// Validate refuses bare onto a destructive target and onto one with a
// required input nothing seeds. What it cannot know is the house rule
// pinned here: one non-Read capability may run on a keypress, and it is
// named. deny is the fail-safe direction, taking nothing and granting
// nothing; anything else mutating in one key is a decision for the
// declaration that names it, not a side effect of setting a flag.
func TestBareIsDeclaredOnlyWhereItIsSafe(t *testing.T) {
	...
	for _, c := range reg.Capabilities() {
		for _, a := range c.Actions {
			...
			if !ok {
				t.Errorf("%s %s: bare on unknown capability %q", c.ID, a.Key, a.Target)
				continue
			}
			if target.Safety != plugin.Read && a.Target != "agent.deny" {
				t.Errorf("%s %s: bare on %s (%s) — a one-key mutation is for the fail-safe direction only",
					c.ID, a.Key, a.Target, target.Safety)
			}
		}
	}
}
