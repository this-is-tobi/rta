...
func TestFramesFitTerminalWidth(t *testing.T) {
	for _, size := range layoutSizes {
		...
		msg := runCmd(context.Background(), 0, c, nil, false, nil, "", nil, config.Connection{}, false)()
		next, _ := m.Update(msg)
		rm := next.(Model)
...
func TestDumpFrames(t *testing.T) {
	...
	c, _ := layoutRegistry(t).Capability("lay.table")
	next, _ := m.Update(runCmd(context.Background(), 0, c, nil, false, nil, "", nil, config.Connection{}, false)())
