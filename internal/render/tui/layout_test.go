...
func sized(t *testing.T, w, h int) Model {
	...
	for i, ti := range model.tiles {
		msg := tileCmd(i, ti, statedConfig{}, "", nil, config.Connection{})()
...
func TestFramesFitTerminalWidth(t *testing.T) {
	for _, size := range layoutSizes {
		...
		msg := runCmd(context.Background(), 0, c, nil, false, statedConfig{}, "", nil, config.Connection{}, false)()
		next, _ := m.Update(msg)
		rm := next.(Model)
...
func TestDumpFrames(t *testing.T) {
	...
	c, _ := layoutRegistry(t).Capability("lay.table")
	next, _ := m.Update(runCmd(context.Background(), 0, c, nil, false, statedConfig{}, "", nil, config.Connection{}, false)())
