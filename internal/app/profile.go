...
func newProfileCommand(reg *registry.Registry, opts *globalOpts) *cobra.Command {
	...
	list := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			t := profileTable(cfg, reg)
			// The table even when nothing is configured, and the sentence
			// beside it for a person: see view.Table.Empty. This swapped in
			// a text view for pretty output, the one place it could see the
			// format, which put the sentence into a pipe too and left -o md
			// a heading row over nothing.
			if len(t.Rows) == 0 {
				t.Empty = "No profile is configured yet — `rta profile set <name> --plugin <plugin> " +
					"--set key=value` writes one, and `rta use <name>` switches it on."
			}
			return render(cmd, t, nil)
		},
	}
	show := &cobra.Command{
		Use:               "show <profile>",
