...
// PendingRemote is the consent.list verb: the queue exactly as the local
// surfaces walk it, tampered ids included — a request rewritten under a
// remote server is the same alarm as under a local one, and the operator
// reading the listing is who the alarm is for.
func PendingRemote() (operatorid.ConsentList, *view.Error) {
	q, err := consent.Scan()
	if err != nil {
		return operatorid.ConsentList{}, view.Errorf("agent.pending.unreadable", "%v", err)
	}
	return operatorid.ConsentList{Waiting: q.Waiting, Tampered: q.Tampered}, nil
}
...
func AnswerRemote(spec operatorid.AnswerSpec, label string) (operatorid.AnswerOutcome, *view.Error) {
