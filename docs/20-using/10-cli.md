...
An empty listing is still its table — the columns, and `rows` an empty array — so `jq '.rows[]'` yields nothing and exits 0, and `-o csv` prints the header. An empty tree is the same, `roots` an empty array rather than `null`, and so is an empty page of sections, `items` an empty array. The sentence saying what would fill it is drawn only for a person: in `pretty` on a terminal, in `-o md`, and in the TUI. `pretty` redirected to a file or a pipe draws the headings instead, so the sentence never lands in a file as though it were a row. A text that is itself the answer is empty in the same way: `rta git diff` on a clean tree says there are no uncommitted changes on a terminal, answers `"body": ""` to `-o json`, and `rta git diff > x.patch` writes an empty file. Under `-o csv` every empty result is its header row alone — a text with no body, a tree with no roots, a chart with no series, a page with no sections — so a script that skips the header never meets a row about nothing.
