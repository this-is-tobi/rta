...
`rta audit clients --fix` prints those edits rather than describing them: the scoped allowlist shape, a `chmod` for a config other accounts can read, a pinned version for a server fetched at launch, a credential moved out of the file that was carrying it — in an `env` block or, for a server declared as a URL to call rather than a binary to launch, in its `headers` — an https endpoint for one being called in clear, the parts of [the container recipe](./20-mcp.md#in-a-container-for-a-hardened-server) a containerised rta is missing, and a deny list for rta's own authority-expanding commands — every plugin whose verbs are all for the person at the terminal (`rta grant`, `rta agent`, `rta lock`, `rta operator`, `rta pkg`), derived from the catalogue so a plugin added tomorrow is on it the day it lands, and every verb of a mixed plugin that is for the person alone (`rta audit clients`, `rta audit doctor`, `rta keys add`, `rta keys backup`, `rta keys restore`), derived the same way, plus `rta kv` whole and `rta plugin trust|allow|install` — so the ordinary reach for self-granting is refused by the agent's own harness before rta ever sees it. It prints and never writes, for the reason the audit itself states: the file that grants an agent access to your secrets changes by your hand. And a string match is a seatbelt, not a wall — none of rta's own gates relax because it exists.
...
| **Consent needs somewhere to go** | Answered, narrowly: the [operator channel](./20-mcp.md#the-operator-channel) carries consent now — `rta agent allow <id> --server`, signed under the operator's passphrase, one-shot included — so `--consent` starts beside `--http` when `--operators` names who answers. Without a roster the refusal stands: a control nobody can exercise must not pretend it works |
