...
- **`Options`** becomes a TUI picker, shell completion, and an `enum` in the MCP schema. Use it for a fixed set. The host holds every value to it before your handler runs: one naming an option in another case arrives spelled the way you declared it, and one naming none of them is refused, on every surface, including a value from the operator's config. A `Default` has to be one of the options.
