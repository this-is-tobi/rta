...
- **`Min`** and **`Max`** bound an `Int` or `Float` the same way: a value outside them is refused with the range named, never moved inside it, so your handler can take the bound as given.
