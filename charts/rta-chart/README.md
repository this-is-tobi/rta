...
![Version: 0.10.0](https://img.shields.io/badge/Version-0.10.0-informational?style=flat-square) ![Type: application](https://img.shields.io/badge/Type-application-informational?style=flat-square) ![AppVersion: 0.25.0](https://img.shields.io/badge/AppVersion-0.25.0-informational?style=flat-square)
...
Nothing else is needed for its plugins to load. The image keeps them, with their **trust**, in its read-only system root at `/usr/local/lib/rta`, which this chart's data volume never touches — so a fresh volume on `/rta-home` hides nothing, and `rta plugin list` in the pod shows every plugin trusted by the image on first start.
...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| image.allowFullImage | bool | `false` | Permit `ghcr.io/this-is-tobi/rta-full`, which the chart otherwise refuses.  The full image carries every first-party plugin and every external tool, which makes it the widest reach rta has - the documentation is explicit that it is the wrong thing to point an agent at, because the image IS the plugin allowlist and this one allows everything. For a person driving a console, or for a homelab where the point is to exercise `kube`, `cnpg` and the rest against real infrastructure, it is the honest choice; for an instance an agent talks to unattended, build a narrow image with the plugins that job needs.  Setting this to `true` has two effects: it lifts the refusal, and it makes `serverDefaults.plugins.allow` meaningful, since only that image's entrypoint reads it. The image's plugins and their trust live in its read-only system root, /usr/local/lib/rta, which the data volume never touches - nothing needs copying into the volume for them to load. |
